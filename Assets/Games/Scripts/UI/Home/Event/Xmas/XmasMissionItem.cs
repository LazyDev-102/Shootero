...
public class XmasMissionItem : MonoBehaviour, IItem<XmasMissionItemData>
{
    ...
    public XmasMissionItemData dataStack { get ; set ; }

    private void Start() {
        claimButton.AddEvent(ClaimReward);
    }

    public void Initialize(XmasMissionItemData data) {
        SetData(data);
        Generate();
    }
    private void SetData(XmasMissionItemData data) {
        dataStack = data;
    }
    public IItem<XmasMissionItemData> Generate() {
    ...
    private void ClaimReward() {
        dataStack.Apply();
        Reload();
        EventDispatcher.Instance.Dispatch(new EventKey.OnClaimXmasMission() { Position = rewardAmountText.transform.position });
    }
    private void Reload() {
