...
public class XmasPackItem : MonoBehaviour, IItem<XmasPackItemData>
{
    ...
    public XmasPackItemData dataStack { get ; set ; }

    private void Start() {
        exchangeButton.AddEvent(Exchange);
    }

    public void Initialize(XmasPackItemData data) {
        SetData(data);
        Generate();
    }
    private void SetData(XmasPackItemData data) {
        dataStack = data;
    }
    public IItem<XmasPackItemData> Generate() {
    ...
    private void Exchange() {
        GameResources.Instance.Inventory.EnoughPrice(dataStack.Price, ()=> {
            ...
            EventDispatcher.Instance.Dispatch(EventKey.OnExchangeXmasCandy);
