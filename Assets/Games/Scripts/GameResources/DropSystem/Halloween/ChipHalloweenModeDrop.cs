...
[CreateAssetMenu(fileName = "ChipHalloweenDrop", menuName = "Resource/HardData/Drop/Halloween/ChipDrop")]
public class ChipHalloweenModeDrop : BaseDrop {
    ...
    private int GetChipInIcon(float halloweenDifficultMultipler) {
        return Mathf.CeilToInt(5 * (0.8f + 0.2f * halloweenDifficultMultipler));
    }
    ...
    public override void Droping(Vector2 position, EnemyType eType) {
        ...
        if (RandomHelper.RandomWithProbability(dropProbablity.Probability)) {
            ...
            int numberChipInIcon = GetChipInIcon(controller.GetDifficultMultiple());
