...
[CreateAssetMenu(fileName = "MaterialHalloweenDrop", menuName = "Resource/HardData/Drop/Halloween/MaterialDrop")]
public class MaterialHalloweenModeDrop : BaseDrop {
    ...
    private void DropMaterial(Vector2 position, int id, int amount) {
        ...
        ItemDropController neưMaterialController = GameManager.Instance.GameLoader.SpawnDropItem(materialDropPrefab, position);
        if (neưMaterialController) {
            neưMaterialController.SetItem(itemRandom, amount);
            neưMaterialController.Initalize();
        }
    }
