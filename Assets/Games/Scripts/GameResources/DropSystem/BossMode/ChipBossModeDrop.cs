...
[CreateAssetMenu(fileName = "ChipBossModeDrop", menuName = "Resource/HardData/Drop/BossMode/ChipDrop")]
public class ChipBossModeDrop : BaseDrop {
    ...
    private int GetChipInIcon(float BossModeMultipler) {
        return Mathf.CeilToInt(5 * (0.8f + 0.2f * BossModeMultipler));
    }
    ...
    public override void Droping(Vector2 position, EnemyType eType) {
        ...
        if (RandomHelper.RandomWithProbability(dropProbablity.Probability)) {
            BossModeController controller = GameManager.Instance.GetGameController<BossModeController>();
            ...
            int numberChipInIcon = GetChipInIcon(controller.CurrentBossModeMultipler);
