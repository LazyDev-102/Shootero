...
[CreateAssetMenu(fileName = "HalloweenModeData", menuName = "Resource/Modes/Halloween/HalloweenModeData")]
public class HalloweenModeData : ScriptableObject {
    ...
    [SerializeField] private HalloweenModeWaveData[] zoneData;
    [SerializeField] private HalloweenZoneData enemyData;
    ...
    public HalloweenModeWaveData[] ZoneData { get => zoneData; }
    public ZoneBackground Background { get => background; }
    public HalloweenZoneData Prefab { get => enemyData; }
    ...
    public HalloweenZoneData EnemyData { get => enemyData;}
    ...
    public void BuyMoreTicket() {
        buyableRemain--;
        GameResources.Instance.Inventory.AddHTicket(moreTicketReward.Amount);
    }
    ...
    public HalloweenModeWaveInfo[] GenerateWaves() {
        HalloweenModeWaveInfo[] waveInfoes = new HalloweenModeWaveInfo[zoneData.Length];
        for (int i = 0; i < waveInfoes.Length; ++i) {
            HalloweenModeWaveInfo waveInfo = zoneData[i].CreateInfo(0, i) as HalloweenModeWaveInfo;
    ...
    private void ResetTurn() {
        if (DateTime.Now.DayOfYear + DateTime.Now.Year * 365 > day + year * 365) {
            buyableRemain = buyablePerDay;
            year = DateTime.Now.Year;
            day = DateTime.Now.DayOfYear;
            var inv = GameResources.Instance.Inventory;
            var hTicket = inv.GetHTicket();
            if (hTicket.Amount < 5) {
                inv.Add(hTicket.Id, 5 - hTicket.Amount);
            }
        }
    }
