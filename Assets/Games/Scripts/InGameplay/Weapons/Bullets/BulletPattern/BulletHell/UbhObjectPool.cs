...
public sealed class UbhObjectPool : UbhSingletonMonoBehavior<UbhObjectPool> {
    ...
    public void RemoveAllBullet() {
        foreach (var item in m_pooledBulletDic) {
            var bullets = item.Value.m_bulletList;
            if (bullets != null) {
                for (int i = 0; i < bullets.Count; i++) {
                    if (bullets[i].gameObject.layer == GameLayer.EnemyBulletIndex || bullets[i].CompareTag(GameTag.EnemyBullet)) {
