...
public class MB10Move : MinibossMove {
    ...
    public void StartMoveAfterAttackMB10(Vector2 vector2) {
        var point = GetPointMoveMB10(vector2);
        ...
        pathPoints[2] = point;
        curMoveTween?.Kill();
        curMoveTween = transform.DOPath(pathPoints, timeMove, PathType.CatmullRom, PathMode.TopDown2D, 5).OnComplete(OnEndMoveAfterAttackMB10).SetEase(attackMoveCurve).OnKill(OnEndMoveBeKill);
    }
    ...
    private void OnEndMoveAfterAttackMB10() {
        isEndMove = true;
    }


    public virtual Vector2 GetPointMoveMB10(Vector2 point) {
