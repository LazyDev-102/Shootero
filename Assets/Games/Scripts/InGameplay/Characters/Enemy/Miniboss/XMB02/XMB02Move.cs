...
public class XMB02Move : MinibossMove {
    ...
    public void StartMoveAfterAttackXMB02(Vector2 vector2) {
        var point = GetPointMoveXMB02(vector2);
        ...
        pathPoints[2] = point;
        curMoveTween?.Kill();
        curMoveTween = transform.DOPath(pathPoints, timeMove, PathType.CatmullRom, PathMode.TopDown2D, 5).OnComplete(OnEndMoveAfterAttackXMB02).SetEase(attackMoveCurve).OnKill(OnEndMoveBeKill);
    }
    ...
    private void OnEndMoveAfterAttackXMB02() {
        isEndMove = true;
    }


    public virtual Vector2 GetPointMoveXMB02(Vector2 point) {
