...
public class MB16Move : MinibossMove {

    #region MB16 Attack Special
    ...
    #region MB16 Attack 2
    [Header("MB16 Attack 2")]
    ...
    public void StartMoveAfterAttackMB16(Vector2 vector2) {
        var point = GetPointMoveMB16(vector2);
        ...
        pathPoints[2] = point;
        curMoveTween?.Kill();
        curMoveTween = transform.DOPath(pathPoints, timeMove, PathType.CatmullRom, PathMode.TopDown2D, 5).OnComplete(OnEndMoveAfterAttackMB16).SetEase(attackMoveCurve).OnKill(OnEndMoveBeKill);
    }

    private void OnEndMoveAfterAttackMB16() {
        isEndMove = true;
    }
    ...
    protected virtual Vector2 GetPointMoveMB16(Vector2 point) {
