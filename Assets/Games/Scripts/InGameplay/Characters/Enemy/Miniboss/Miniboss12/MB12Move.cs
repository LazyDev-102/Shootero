...
public class MB12Move : MinibossMove {
    ...
    public void StartMoveAfterAttackMB12(Vector2 vector2) {
        var point = GetPointMoveMB12(vector2);
        ...
        pathPoints[2] = point;
        curMoveTween?.Kill();
        curMoveTween = transform.DOPath(pathPoints, timeMove, PathType.CatmullRom, PathMode.TopDown2D, 5).OnComplete(OnEndMoveAfterAttackMB12).SetEase(attackMoveCurve).OnKill(OnEndMoveBeKill);
    }

    private void OnEndMoveAfterAttackMB12() {
        isEndMove = true;
    }
    ...
    public virtual Vector2 GetPointMoveMB12(Vector2 point) {
