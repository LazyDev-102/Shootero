...
public class B05Move : BossMove {
    ...
    public void StartMoveAfterAttackB05(Vector2 vector2) {
        var point = GetPointMoveB05(vector2);
        ...
        pathPoints[2] = point;
        curMoveTween?.Kill();
        curMoveTween = transform.DOPath(pathPoints, timeMove, PathType.CatmullRom, PathMode.TopDown2D, 5).OnComplete(OnEndMoveAfterAttackB05).SetEase(attackMoveCurve).OnKill(OnEndMoveBeKill);
    }
    ...
    private void OnEndMoveAfterAttackB05() {
        isEndMove = true;
    }
    protected virtual Vector2 GetPointMoveB05(Vector2 point) {
