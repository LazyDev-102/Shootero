...
public class HalloweenModeController : GameController {
    private HalloweenModeWaveInfo[] waveInfoes;
    private int currentWaveIndex;
    private int currentZoneIndex;

    private HalloweenModeWaveSpawner waveSpawner;
    private HalloweenModeData halloweenData;

    public HalloweenModeWaveInfo CurrentWaveInfo => waveInfoes[currentWaveIndex];
    public HalloweenModeWaveData CurrentWaveData => CurrentWaveInfo.HalloweenWaveData;

    public bool IsCompleted => currentWaveIndex + 1 >= waveInfoes.Length;

    public HalloweenModeController(GameManager manager) : base(manager) {
    }

    public override void Initialize() {
        halloweenData = GameResources.Instance.Halloween;
        currentWaveIndex = 0;
        waveInfoes = halloweenData.GenerateWaves();
        BackgroundManager.Instance.SetBackground(halloweenData.Background);
    }
    ...
    public override void Lose() {
        EventDispatcher.Instance.Dispatch(EventKey.HalloweenComplete);
        PopupHUD.Instance.Show<HalloweenResultPopup>()
            .SetWin(false)
    ...
    public override void Win() {
        EventDispatcher.Instance.Dispatch(EventKey.HalloweenComplete);
        EventDispatcher.Instance.Dispatch(EventKey.HalloweenCompleteWin);
    ...
    private void CreatWave() {
        ...
        HalloweenModeWaveSpawner newSpawner = CurrentWaveInfo.SetupSpawner(waveSpawner) as HalloweenModeWaveSpawner;
    ...
    public override float GetDifficultMultiple() {
        return CurrentWaveInfo.GetWaveMultipler() * halloweenData.MultiDifficult;
    }
